...
def get_report(date_from: str, date_to: str) -> list[dict]:
    ...
    if resp.status_code in (401, 403):
        raise ValueError(
            f"❌ Ошибка доступа к WB API ({resp.status_code}). "
            "Убедись что у ключа включён раздел 'Статистика' в личном кабинете WB."
        )
...
# JOB FUNCTIONS
# ──────────────────────────────────────────────────────────────
async def send_report(bot: Bot, period_label: str, date_from: str, date_to: str, reply_to=None):
    """Получить данные с ВБ и отправить отчёт."""
    log.info(f"Формирую отчёт: {date_from} — {date_to}")

    async def notify(text, parse_mode=None):
        """Отправить сообщение в CHAT_ID, и если команда от другого чата — туда тоже."""
        await bot.send_message(chat_id=CHAT_ID, text=text, parse_mode=parse_mode)
        if reply_to and str(reply_to.chat_id) != str(CHAT_ID):
            await reply_to.reply_text(text, parse_mode=parse_mode)

    await notify(f"⏳ Формирую {period_label} отчёт WB за {date_from} — {date_to}...")
    try:
        ...
        if not rows:
            await notify(
                f"ℹ️ Нет данных за период {date_from} — {date_to}. "
        ...
        await notify(msg, parse_mode=ParseMode.MARKDOWN)

        # Алерты отдельным сообщением если есть
        if alerts:
            alert_text = "🚨 *ВНИМАНИЕ — ШТРАФЫ И УДЕРЖАНИЯ*\n\n" + "\n".join(alerts)
            await notify(alert_text, parse_mode=ParseMode.MARKDOWN)
        ...
        err_text = (
            f"❌ Ошибка при получении данных с WB:\n`{e}`\n\n"
            "Проверь API-ключ и повтори /report"
        )
        await notify(err_text, parse_mode=ParseMode.MARKDOWN)
...
async def cmd_report(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await send_report(context.bot, "Еженедельный", date_from, date_to, reply_to=update.message)
...
async def cmd_today(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await send_report(context.bot, "Дневной", today, today, reply_to=update.message)
